...
def  main():
  ...
  installer = "\installer\NESting Installer.exe"
   
  if demo:
    installer = "\installer\NESting Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\NESting manual.pdf" 
  ]
  ...
  ZIPNAME = "NESting-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "NESting-v" + FULLVERSIONSTR + "-win-demo.zip"
