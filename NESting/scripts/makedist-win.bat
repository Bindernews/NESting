...
if %1 == 1 (echo Making NESting Windows DEMO VERSION distribution ...) else (echo Making NESting Windows FULL VERSION distribution ...)
...
REM - msbuild NESting-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild NESting.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild NESting.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\NESting.aaxplugin\Contents\Win32\NESting.aaxplugin --out .\build-win\aax\bin\NESting.aaxplugin\Contents\Win32\NESting.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\NESting.aaxplugin\Contents\x64\NESting.aaxplugin --out .\build-win\aax\bin\NESting.aaxplugin\Contents\x64\NESting.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\NESting.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\NESting.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "NESting Installer" ".\installer\NESting Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\NESting Installer.exe" ".\installer\NESting Demo Installer.exe"
REM -del ".\installer\NESting Installer.exe"
REM -)
